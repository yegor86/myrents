...
class LanguageDetect
{
    ...
    public function MRdetect($string=''){
	    $detected = 'ru'; //по умолчанию  считаем язык русским
	    try {
		$this->omitLanguages(array('english','russian'),true); //устанавливаем искомые языки, чтобы лишние не попадали в выборку
		$this->setNameMode(2);		             //мод отображения - двубуквенный
		$result = $this->detect($string);			//определяем язык
