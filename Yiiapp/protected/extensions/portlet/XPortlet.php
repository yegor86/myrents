...
class XPortlet extends CWidget
{
	...
	public function init()
	{
		if($this->visible)
		{
			...
			$this->_assetsUrl = Yii::app()->assetManager->publish(dirname(__FILE__).DIRECTORY_SEPARATOR.'assets', false, -1, YII_DEBUG);
			if($this->cssFile===null)
			{	
				$cssFile= $this->_assetsUrl.'/portlet.css';
				$cs->registerCssFile($cssFile);
			}
			else if($this->cssFile!==false)
				$cs->registerCssFile($this->cssFile);
			
			$cs->registerScriptFile($this->_assetsUrl.'/js/xportlet.js');
                        $cs->registerScriptFile($this->_assetsUrl.'/js/jquery.keyboard.js');
