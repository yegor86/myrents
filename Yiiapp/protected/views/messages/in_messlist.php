...
<?php $exp = explode(' ', $message->date);?>
		...
		<?php echo CHtml::link($message->sender->firstname . ' ' . $message->sender->lastname, '/user/' . $message->sender->id, array('class' => 'link')) ?>
            <span style="position:absolute;right:50px;margin-top:-2px;font-size:11px;"><?php echo $exp[0] ?> <?php echo $exp[1] ?></span> <?php echo CHtml::link(Yii::t('messages','message.reply'), "/messages/send/".$message->sender->id."",array('class'=>'btnreply'))?> <?php echo MRChtml::ajaxLink('', "/user/".$user->id."/messages/in", array(
		    'update' => '#subcontent',
		    'updateUrl' => true,
