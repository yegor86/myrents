...
<script>
    $(function(){
        ...
        if($('#rentertype').val()=='agency'){
                $('#editname').text('<?php echo Yii::t('default', 'agency.create.name'); ?>'); //заменяем label
                $('#lastname_box').hide(); //скрываем блок lastname
                $('#lastname_box input').val('');
                $('#hint_firstname').text('<?php echo Yii::t('default', 'agency.create.name.hint'); ?>'); //заменяем Hint
        }else{
                $('#editname').text('<?php echo Yii::t('default', 'name'); ?>'); //возвращаем label
                $('#lastname_box').show(); //показываем блок lastname
                $('#hint_firstname').text('<?php echo Yii::t('default', 'user.edit.firstname.length'); ?>'); //возвращаем Hint
        }
        $('#cuselFrame-rentertype').change(function(){
            if($('.cuselActive').attr('val')=='agency') {
                $('#editname').text('<?php echo Yii::t('default', 'agency.create.name'); ?>'); //заменяем label
                $('#lastname_box').hide(); //скрываем блок lastname
                $('#lastname_box input').val('');
                $('#hint_firstname').text('<?php echo Yii::t('default', 'agency.create.name.hint'); ?>'); //заменяем Hint
				...
				<div class="input_div">
                                    <b class="stl_2"><br /><?php echo $form->label($editform, 'firstname', array('id'=>'editname')); ?>:</b><br />
				    <?php echo $form->textField($editform, 'firstname'); ?>
				    <?php echo $form->error($editform, 'firstname'); ?>
				</div>
				<div class="hint" id="hint_firstname"><?php echo Yii::t('default','user.edit.firstname.length')?></div>
				
				<div class="input_div" id="lastname_box">
				...
				<b class="stl_2"><?php echo $form->labelEx($editform, 'rentertype'); ?>:</b><br />
				<div style="width:200px">
				    <?php echo MRChtml::enumDropDownList($editform, 'rentertype',array('id'=>'rentertype', 'style'=>'width:200px')); ?>
				    <?php echo $form->error($editform, 'rentertype'); ?></div>
