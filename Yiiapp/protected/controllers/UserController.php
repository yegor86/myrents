...
class UserController extends MyRentsController {
	...
	if ($id) {
		...
		$criteria = new CDbCriteria(array(
			    'condition' => '`todo`=:todo AND `user`=:user ',
		...
		$viewparams['rents'] = Rent::model()->fullparms($lang->id)->findAll($criteria);
		//$viewparams['rents'] = Rent::model()->texted($lang->id)->findAll($criteria);
	    } else {  
		$viewparams['rents'] =false;
		...
		if ($viewparams['user']) {
		    ...
		    $viewparams['pagination'] = Rent::paginateCriteria($criteria,'Rent');
		    $viewparams['rents'] = Rent::model()->
                            fullparms($lang->id)->is_full()->findAll($criteria);
		}
	    }
	...
	if ($id && ($id == Yii::app()->user->id)) {
	    ...
	    $viewparams['user'] = User::model()->with(array(
			'favorites'=>array(
                            'scopes'=>array('is_full','texted'=>$this->curlang->id)
                        )
		    ))
		    ->findByPk($id);
	    if (Yii::app()->request->isAjaxRequest) {
