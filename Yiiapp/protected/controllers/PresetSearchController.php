	...
	//	Yii::app()->clientScript->registerMetaTag($text->keywords, 'keywords');
	  //  }
	    ...
	    array_unshift($cityList,array('id'=>0,
            ...
            $regionList = CustomFunctions::RegionsForCity($searchForm->city);
	    $top = Rent::model()->with('top')->fullparms($this->curlang->id) ->findAllByPk(TopFunc::init()->getTopIds($searchForm->todo,$searchForm->type),array('order'=>'`top`.`start` DESC'));
