...
class SmsController extends MyRentsController {
	...
	$selector = ($tempSMS->action=='t')?'smsTopPeriods':'smsMainPeriods';
	if(isset(Yii::app()->params[$selector][$tempSMS->number]))
	    {
	    $phoneNumber = Yii::app()->params[$selector][$tempSMS->number];
	    $rent = Rent::model()->with('renter')->findByPk($tempSMS->rent_id);
	    if($rent){
		TopFunc::init()->addToTop($rent->id,
			$phoneNumber['periodInDays'],
			$tempSMS->action);
		$result = true;
		$opLog = new OperationLog();
		$opLog->operation_id = 5;
		$opLog->user_id = $rent->renter->id;
		$opLog->comment = 'add rent '. $rent->id .' to '.$tempSMS->action .' for'. $phoneNumber['periodInDays']. 'days (number'.$tempSMS->number .')';
		$opLog->save();
		$rent->renter->notify(Yii::t('default','rent.was.topped',array(':rentId'=>$rent->id)));
	    } else{
                Yii::log($vars['sms_id']." rent not found", 'info','SMS');
            }
	} else{
            Yii::log($vars['sms_id']." wrong number", 'info','SMS');
        }
	return $result;
    }
