...
class SearchController extends MyRentsController {
	...
	if (isset($_GET['SearchForm'])) {  //если был запрос на поиск
	    $searchForm->attributes = $_GET['SearchForm']; //заполняем  модель формы
            ...
            // если был переход, проверяем локализацию введённых ранее параметров
        /*    if (!Yii::app()->request->isAjaxRequest) {
                $l=new LanguageDetect();
            if($searchForm->city){
                 $detected = $l->MRdetect($searchForm->city); //проверка ввода языка
		   if($detected!=Yii::app()->locale){
                       $searchForm->city ='';$searchForm->region ='';
                   }
	...
	//Yii::log(CVarDumper::dumpAsString(TopFunc::init()->getTopIds($searchForm->todo, $searchForm->type)) , 'trace' );
	$top = Rent::model()->mostComplete($this->curlang->id)
                ->is_full()->with('top')
                ->findAllByPk(TopFunc::init()->getTopIds($searchForm->todo, $searchForm->type),array(
	...
	if (Yii::app()->request->isAjaxRequest) {   //если запрос был аяксовый, то возвращаем только результат поиска
	    ...
	    $todoArray = $this->modelsNamestoArray(Todo::model()->cache(Yii::app()->params['cachetime'])
             ...
             $cityList =SearchEngine::engine()->getNetsedItemsByLevel(2,  Yii::app()->language); 


        
        array_unshift($cityList,array('id'=>0,
        ...
        $regionList = array();
	if(!in_array($searchForm->city, CustomFunctions::ArrayFromSubitems($cityList, 'name')))
		$searchForm->city=Yii::t('default','any');
        else $regionList = CustomFunctions::RegionsForCity($searchForm->city);
            //список городов
	    //SEO block
