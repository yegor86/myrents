...
public $layout = 'mainlayout';
	...
	$selectedRentsIds =array(); //тут выбран список на главную
	$selectedRentsIds = TopFunc::init()->getMainPageIds();
	$selectedRents = count($selectedRentsIds)?
		Rent::model()->fullparms($this->curlang->id)->with('renter')
                ->findAll('`t`.`id` IN ('.  implode(',', $selectedRentsIds).')')
		:array();
	//делаем случайный сдиг.
	$selectedRents =  CustomFunctions::RoundedArraySlise($selectedRents, rand(0, count($selectedRents)), count($selectedRents));
