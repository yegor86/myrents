...
class RentController extends MyRentsController {
	...
	if ($id) {
	    ...
	    //используем Arent вместо Rent, потому-что надо выбирать и удалённые тоже
	    $rent = Rent::model()->resetScope()->mostComplete($lang->id)->findByPk($id);
	    if(!$rent){
			header("HTTP/1.0 404 Not Found");
			$this->assignAndRender('404_rent_not_found');
	    }
	    elseif($rent->is_deleted){
			header("HTTP/1.0 410 Gone");
			$this->assignAndRender('404_rent_not_found');
	    }
	    elseif(!$rent->in_show&&($rent->renter->id != Yii::app()->user->id)){
	...
	if (isset($_POST['RentComment']) && (!Yii::app()->user->isGuest) && ($this->user->active)&&(Yii::app()->user->getState('roles')!='banned'&&Yii::app()->user->getState('roles')!='reader')) {
