...
class VKontakteOAuthService extends EOAuth2Service {	
	...
	protected function getTokenUrl($code) {
	    return $this->providerOptions['access_token'].'?client_id='.$this->client_id.'&client_secret='.$this->client_secret.'&code='.$code.'&redirect_uri='.Yii::app()->request->getHostInfo().Yii::app()->request->getUrl();
	}
	
	protected function getAccessToken($code) {
		$result = parent::makeRequest($this->getTokenUrl($code));;
	 ...
	 * вконтакте изменил порядок ошибки. пришлось прикрутить костыль isset
	...
	protected function fetchJsonError($json) {
		if (isset($json->error)) {
		    $returnArray = array();
		    $returnArray['code']=(isset($json->error->error_code))?$json->error->error_code:500;
		    $returnArray['message']=(isset($json->error->error_msg))?$json->error->error_msg:$json->error;
			return $returnArray;
		}
		else
			return null;
