...
class SearchEngine {
	...
	if ($rent->adress) {
	    ...
	    $photosCondition = (count($hasPhotos)) ? 'AND (`t`.`id` IN(' . implode(',', $hasPhotos) . '))' : '';
	...
	//если был текстовый выбор, получаем IDшники и добавляем в поиск
	...
	if($searchText||$searchCity){
	    ...
	    $searchText = ($searchText)?'*'.implode($implodeSymbol,  $regexp_matches[1]).'*':'';
	    ...
	    $searchCity = implode($implodeCitySymbol,$regexp_matches[1]);
	    unset ($regexp_matches);
		...
		//если ничего небыло найдено
	     if (!count($rents_ids)){
		 if(!$or) { //если мы искали не по запросу "или", повторяем запрос с "или"
		     $this->addParamsByTextString(true);
		 }else{ //если это и был запрос с "или", значит мы уже второй раз ничего не нашли - возвращаем пустой ответ
		     $rents_ids = array(0);
